...
public class Decryptor implements Runnable {
    ...
    private Buffer<MessagePOJO> outputBuffer;
    private Buffer<byte[]> inputBuffer;

    public Decryptor(Buffer<byte[]> inputBuffer, Buffer<MessagePOJO> outputBuffer) {
    ...
    public void run() {
        try {
            while (true) {
                byte[] rawData = inputBuffer.take();

                try {
                    MessagePOJO message = decrypt(rawData);
            ...
            System.out.println("Decryptor stoped");
